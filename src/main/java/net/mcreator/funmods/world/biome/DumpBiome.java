...
import net.mcreator.funmods.block.TrashBlockBlock;
import net.mcreator.funmods.FunModsModElements;

@FunModsModElements.ModElement.Tag
public class DumpBiome extends FunModsModElements.ModElement {
	public static Biome biome;
	public DumpBiome(FunModsModElements instance) {
		super(instance, 86);
		...
		public void registerBiomes(RegistryEvent.Register<Biome> event) {
			if (biome == null) {
				...
				BiomeGenerationSettings.Builder biomeGenerationSettings = new BiomeGenerationSettings.Builder()
						.withSurfaceBuilder(SurfaceBuilder.DEFAULT.func_242929_a(new SurfaceBuilderConfig(TrashBlockBlock.block.getDefaultState(),
								TrashBlockBlock.block.getDefaultState(), TrashBlockBlock.block.getDefaultState())));
				...
				biome = new Biome.Builder().precipitation(Biome.RainType.RAIN).category(Biome.Category.NONE).depth(0.19f).scale(0.2f)
						.temperature(0.5f).downfall(0.5f).setEffects(effects).withMobSpawnSettings(mobSpawnInfo.copy())
						.withGenerationSettings(biomeGenerationSettings.build()).build();
				event.getRegistry().register(biome.setRegistryName("fun_mods:dump"));
			}
		}
	}
	@Override
